...
import org.lokray.nebula.ast.Program;
import org.lokray.nebula.ast.declarations.*;
import org.lokray.nebula.ast.expressions.*;
import org.lokray.nebula.ast.statements.*;
import org.lokray.nebula.lexer.Token;
import org.lokray.nebula.lexer.TokenType;
import org.lokray.nebula.util.CompilerConfig;
import org.lokray.nebula.util.Debug;
...
public class SemanticAnalyzer implements ASTVisitor<Type>
{
	...
	public void analyze(Program program)
	{
		...
		{
			Debug.log("--- Semantic Analysis: Integrating Preloaded Symbols ---");
		...
		Debug.log("--- Semantic Analysis: First Pass - Class Discovery ---");
		...
		Debug.log("--- Semantic Analysis: Second Pass - Member Definition ---");
		for (NamespaceDeclaration nsDecl : program.getNamespaceDeclarations())
		{
			String oldNs = this.currentNamespacePrefix;
			String rawNamespaceName = getQualifiedNameFromExpression(nsDecl.getNameExpression());
		...
		Debug.log("--- Second Pass Complete ---");

		// --- REVISED: Phase 3 - Name Mangling ---
		Debug.log("--- Semantic Analysis: Third Pass - Name Mangling ---");
		...
		Debug.log("--- Fourth Pass Complete ---");
		...
		Debug.log("--- Semantic Analysis: Final Pass - Body Analysis ---");
		program.accept(this);

		// --- NEW: OWNERSHIP ANALYSIS PASS ---
		if (config.isOwnershipOptimizationEnabled())
		{
			Debug.log("--- Semantic Analysis: Ownership/Escape Analysis Pass ---");
			performOwnershipAnalysis(program);
			Debug.log("--- Ownership Analysis Complete ---");
		}
		else
		{
			Debug.log("--- Ownership Analysis SKIPPED (disabled in config) ---");
		}
	...
	public Type visitClassDeclaration(ClassDeclaration declaration)
	{
		...
		Debug.log("DEBUG: visitClassDeclaration: Looking up class '" + fullClassName + "'");
